...
const SearchPage = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto my-10">
            <div className="flex flex-col md:flex-row justify-between gap-10">hi
                <FilterPage />
                <div className="flex-1">
                    <div className="flex items-center gap-2">
                        <Input type="text" value={searchQuery} placeholder="Search by restaurant and cuisines"
                            onChange={(e) => setSearchQuery(e.target.value)}

                        />
                        <Button className="bg-orange-500 hover:bg-orange-500">Search</Button>
                    </div>
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-2 my-3">
                        <h1 className="font-medium text-lg">(2)Search result found</h1>
                        <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
                            {
                                ["biryani", "momo", "jalebi"].map((selectedFilter: string, idx: number) => (
                                    <div key={idx} className="relative inline-flex items-center max-w-full">
                                        <Badge className="text-[#D19254] rounded-md hover:cursor-pointe pr-6 whitespace-nowrap" variant={"outline"}>{selectedFilter}</Badge>
                                    ...
                                    [1,2,3].map((item:number,idx:number)=>(
                                        ...
                                        <CardContent className="p-4">
                                            ...
                                            <div className="flex gap-2 mt-4 flex-wrap">
                                                ...
                                                {
                                                    ["biryani", "momo", "jalebi"].map((cuisine: string, idx: number) => (
